...
def get_model(point_cloud, is_training, num_class, sigma, bn_decay=None, weight_decay = None):
    ...
    l5_xyz, l5_points = feature_encoding_layer(l4_xyz, l4_points, npoint=18, radius = 0.9, sigma = 10 * sigma, K=32, mlp=[512,512,1024], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer5')

    # Feature decoding layers
    l4_points = feature_decoding_layer(l4_xyz, l5_xyz, l4_points, l5_points, 0.9, 10 * sigma, 16, [1024,1024], is_training, bn_decay, weight_decay, scope='fa_layer1')
    l3_points = feature_decoding_layer(l3_xyz, l4_xyz, l3_points, l4_points, 0.8, 8 * sigma, 16, [512,512], is_training, bn_decay, weight_decay, scope='fa_layer2')
    l2_points = feature_decoding_layer(l2_xyz, l3_xyz, l2_points, l3_points, 0.4, 4 * sigma, 16, [256,256], is_training, bn_decay, weight_decay, scope='fa_layer3')
    l1_points = feature_decoding_layer(l1_xyz, l2_xyz, l1_points, l2_points, 0.2, 2 * sigma, 16, [256,128], is_training, bn_decay, weight_decay, scope='fa_layer4')
    l0_points = feature_decoding_layer(l0_xyz, l1_xyz, l0_points, l1_points, 0.1, sigma, 16, [128,128,128], is_training, bn_decay, weight_decay, scope='fa_layer5')
